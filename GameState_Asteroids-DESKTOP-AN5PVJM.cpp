...
static AEVec2		PLAYER_SIZE = {50.0f, 25.0f};		// ship size
static AEVec2		GUN_SIZE = { 50.0f, 10.0f };		// ship size

const float			PLAYER_ACCEL_FORWARD = 200.0f;		// ship forward acceleration (in m/s^2)
const float			PLAYER_ACCEL_BACKWARD = 200.0f;		// ship backward acceleration (in m/s^2)
...
void GameStateAsteroidsLoad(void)
{
	...
	AEGfxTriAdd(
		...
		0.5f, -0.5f, 0xFFFFFFFF, 1.0f, 1.0f);*/
	AEGfxTriAdd(
		0.0f, -0.5f, 0xFFFF0000, 0.0f, 1.0f,
...
void GameStateAsteroidsUpdate(void)
{
	...
	// Coordinate conversion from window coordinates to graphics coordinates
	mouseX -= (AEGfxGetWinMaxX() - AEGfxGetWinMinX()) / 2;
	mouseY -= (AEGfxGetWinMaxY() - AEGfxGetWinMinY()) / 2;
	mouseY = -mouseY; // Invert y axis sign 
	float direction = atan2(mouseY - PlayerBody->posCurr.y, mouseX - PlayerBody->posCurr.x);
	PlayerGun->dirCurr = direction;
			...
			for (unsigned long j = 0; j < GAME_OBJ_INST_NUM_MAX; j++)
			{
				...
				case TYPE_PLATFORM:
					if (CollisionIntersection_RectRect(pInst->boundingBox, pInst->velCurr, pOtherInst->boundingBox, pOtherInst->velCurr)) {
						...
						/*AEVec2 newVel{0,0}, normVec{0,0};
						normVec = { -pOtherInst->boundingBox.max.y, 0 };
						AEVec2Normalize(&normVec, &normVec);
						newVel.x = pInst->velCurr.x - 2 * (AEVec2DotProduct(&pInst->velCurr, &normVec)) * normVec.x;
						newVel.y = pInst->velCurr.y - 2 * (AEVec2DotProduct(&pInst->velCurr, &normVec)) * normVec.y;
						pInst->velCurr = newVel;
						//float direction = atan2(pInst->velCurr.y, pInst->velCurr.x);
						//pInst->dirCurr = direction;*/
					}
					break;
				case TYPE_PLAYER:
					continue;
